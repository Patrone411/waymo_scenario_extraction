...
def main() -> int:
    ...
    ap.add_argument("--prefix", default="results/matching_fresh/phaseB/CCrb.osc/")
    ...
    ap.add_argument("--plot", action="store_true", help="Einzelplots anzeigen (GUI nötig)")
    ...
    ap.add_argument(
        "--auto_xlim",
        action="store_true",
        help="Wenn gesetzt: x-Achse pro Plot an min/max der Daten anpassen (nur wenn kein fixed_range gesetzt ist).",
    )
    ...
    # NEUE FELDER (wie von dir angegeben) – aktuell 7 Stück:
    fields = {
        "dist_start_m": ("Anfangsabstand", "Abstand zu Beginn [m]", None),
        "dist_end_m": ("Endabstand", "Abstand am Ende [m]", None),
        "ttc_min_same_lane_s": ("Minimaler TTC", "TTC [s]", (0.0, 60.0)),
        ...
        "window_dur_s": ("Szenariodauer", "Szenariofenster Dauer [s]", None),
        "npc_sddot_end_m": ("NPC-Beschleunigung (t1)", "Beschleunigung [m/s²]", (-10.0, 10.0)),
    }
    ...
    if args.plot:
        for fname, (title, xlabel, fixed_range) in fields.items():
            edges, counts = hist_data[fname]
            if not counts:
                continue
            ...
            plt.figure()
            plt.bar(centers, counts, width=widths, align="center", facecolor="blue", alpha=0.5)
            plt.xlabel(xlabel)
            plt.ylabel("Anzahl")
            plt.title(title)

            if fixed_range is not None:
                plt.xlim(fixed_range[0], fixed_range[1])
            elif args.auto_xlim:
