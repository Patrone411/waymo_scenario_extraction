...
def _presence_from_lane(lane_list, T: int) -> Optional[np.ndarray]:
    """
    Presence = 1 when lane is finite and, after rounding, != 0; else 0.
    Pads/crops to length T. Returns None if lane_list is missing/empty.
    """
    if lane_list is None:
        return None
    arr = _pad_or_crop(_to_np(lane_list, dtype=float), T, fill=np.nan)
    if arr is None:
        return None
    lef = np.asarray(arr, dtype=float)
    mfin = np.isfinite(lef)
    Li = np.zeros_like(lef, dtype=int)
    if np.any(mfin):
        Li[mfin] = np.rint(lef[mfin]).astype(int)
    pres = np.zeros((lef.shape[0],), dtype=float)
    pres[mfin & (Li != 0)] = 1.0
    return pres
...
class TagFeatures:
    ...
    ttc: Dict[Tuple[str, str], np.ndarray] = field(default_factory=dict)           # seconds  <-- ADD
    ...
    def from_tag_json(cls, data: Dict[str, Any], seg_id: str) -> "TagFeatures":
        ...
        for a in actors:
            ...
            #choose lane membership for presence checks
            pres = _presence_from_lane(a_seg.get("osc_lane_id"), T)
            present[a] = pres
    ...
    def load_all_segments(cls, data: Dict[str, Any], min_lanes: Optional[int] = None) -> Dict[str, "TagFeatures"]:
        ...
        # Your semantics:
        # - min_lanes in (1, 2) => exactly 2
        # - min_lanes == 3      => exactly 3
        ...
        if min_lanes is not None:
            ...
            if ml in (1, 2):
                ...
                # optional: either disable filtering or treat as exact ml
                target_lanes = ml

        for seg_id, meta in road.items():
            nlanes = int((meta or {}).get("num_lanes", 0))
            
            # Exact match filter
            if target_lanes is not None and nlanes != target_lanes:
                continue
